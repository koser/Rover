...
## Steps
1. [x] **Analyze**: Read `logs/agent_schedule.json`.
2. [x] **Calculate (Diary/Checkup)**: If 24h passed, trigger `run_agent.sh` for Diary and SystemCheckup with the `--once` flag.
3. [x] **Calculate (GitAgent)**:
...
4. [x] **Execute**: Run the agents as required.
5. [x] **Update Schedule**: Record new `last_run` or `GitAgent_push` timestamps.
6. [x] **Log**: Update the system diary.
