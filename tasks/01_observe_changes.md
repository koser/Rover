...
## Steps
1. [ ] **Analyze**: List all files in the root, `tasks/`, `skills/`, and `agents/` directories.
2. [ ] **Identify**: Compare the current file list and timestamps against the last known state (if available).
3. [ ] **Log**: Use the `system-event-logger` skill to record each change as a `FILE_EVENT` in `logs/diary.log`.
4. [ ] **Self-Check**: Log your own wake-up cycle as an `AGENT_HEARTBEAT`.
